...
| **Human in the loop** | Gate 1 (understanding): confirm / edit / reject what the agent understood — before any tool spend. Gate 2 (plan): approve / edit / reject before save. Both are LangGraph `interrupt`s resumed with `Command(resume=…)`. |
