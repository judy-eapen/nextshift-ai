...
Instrumented every node, model and tool call (developer mode shows the timeline). Baseline vs after on the same five cases: professional 135 → 110 s; student 515 → 354 s (no ideas), 687 → 363 s (three ideas), 525 → 401 s (BLS down), 281 → 82 s (reviewer down). Interview answers ≈ 2–3 s; skip/not-sure instant; understanding 8 s; first cards ≈ 40 s. Main change: first-round cards are lightweight (official outlook + description + your words, fast review); the thinking reviewer and task-level AI evidence run only for the careers you react to. Full student sweep after the change: **24/24**, median 371 s (was ≈ 9–10 min), ≈ $0.032 per exploration (was ≈ $0.07). Full numbers: `evals/results/perf_baseline.json` / `perf_after.json`; design notes in `graph/DESIGN.md` (PERFORMANCE). Remaining bottleneck: the thinking reviewer (≈ 60–90 s per pass on shortlist and deep dive).
